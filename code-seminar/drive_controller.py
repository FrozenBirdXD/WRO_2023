...
class DriveController:
    ...
    def drive_until(self, speed, color: Color):
        while (
            self.left_color_sensor.color() is not color
            and self.right_color_sensor.color() is not color
        ):
    ...
    def line_tracer_distance(self, side: str, distance: int, speed=200):
        ...
        while abs(self.right_motor.angle()) < abs(angle_degrees):
            ...
            self.__drive(speed, correction * multiplier)
    ...
    def turn(self, speed, angle_degrees):
        # Calculate distance
        wheel_travel_distance = (
            (angle_degrees * 2 / 360) * 3.141 * self.WHEEL_BASE_WIDTH
        )

        self.left_motor.run_angle(
            speed, wheel_travel_distance, then=Stop.HOLD, wait=False
        )
        self.right_motor.run_angle(
            speed, wheel_travel_distance, then=Stop.HOLD, wait=True
        )

    def u_turn(self, speed, x, factor):
        r_main = abs(x) / 2
        ...
        if x > 0:
            self.__curve_u_turn(
                speed, abs(turn_rate), 1, motor_angle_small, motor_angle_big
            )
        elif x < 0:
            self.__curve_u_turn(
                speed, 1, abs(turn_rate), motor_angle_big, motor_angle_small
            )
    ...
    def turn_after(self, x, y, direction, speed=1000):
        ...
        if y > 0:
            if direction == "left":
                self.u_turn(speed - 400, -200, 0.25)
            else:
                self.u_turn(speed - 400, 200, 0.25)
        elif y < 0:
            if direction == "left":
    ...
    def __curve_u_turn(
        self, speed, right_turn_rate, left_turn_rate, right_deg, left_deg
    ):
        left_speed = speed
        right_speed = speed
        ...
        if right_deg > left_deg:
            ...
            self.right_motor.run_angle(
                -right_speed, right_deg, then=Stop.HOLD, wait=True
            )
        else:
            self.right_motor.run_angle(
